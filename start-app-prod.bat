...
echo %CYAN%[1/5]%RESET% Verificando base de datos...
...
echo %CYAN%[2/5]%RESET% Verificando datos de seed...
cd backend
call npm.cmd exec ts-node scripts/seed-admin.ts
cd ..
echo %GREEN%Seed verificado.%RESET%

echo %CYAN%[3/5]%RESET% COMPILANDO FRONTEND (esto tomará unos segundos)...
cd frontend
...
echo %CYAN%[4/5]%RESET% Iniciando Backend (NestJS) en puerto 3000...
start "Backend - NestJS (Puerto 3000)" cmd /c "cd backend && npm.cmd start"

timeout /t 3 /nobreak >nul

echo %CYAN%[5/5]%RESET% Iniciando Frontend (Next.js - MODO PRODUCCION) en puerto 3001...
:: IMPORTANTE: next start usa los archivos pre-compilados de .next/
:: No recompila nada en tiempo real. Todo quedo compilado en el paso 3.
...
echo - Todo el frontend ya esta compilado (paso 3)
